...
export default function PhieuNhapKho(props){
    ...
    useEffect(() => {
        switch (mode){
            case 1:
                ...
                break;
            case 2:
                setSearchLabel("Hóa đơn");
                break;
            case 3:
                setSearchLabel("Ngày nhập");
                break;
            default:
                setSearchLabel("Id");
                break;
    ...
    data={ nodes: data.nodes.filter((item) =>{
                switch (mode){
                    ...
                    case 2:
                        return  item.receipt.id.toLowerCase().includes(search.toLowerCase())
                    case 3:
                        return  new Date(item.dateOfImport).toLocaleString('En-GB', {hour12: false}).toLowerCase().includes(search.toLowerCase())
                    default:
                        return item.id.toLowerCase().includes(search.toLowerCase())
    ...
    const COLUMNS = [
        ...
        { label: 'Hóa đơn nhập hàng', renderCell: (item) => <Link className="detail-nav" to={`/HoaDonNhapHang/${item.receipt.id}`}>{item.receipt.id} </Link>, sort: { sortKey: "idhoadon" } },
    ];
            ...
            <Container fluid className="d-flex flex-md-row flex-column justify-content-between align-items-center">
                ...
                <InputGroup style={{width:"400px"}} className="mb-3 rounded-0">
                    ...
                    >
                        ...
                        <Dropdown.Item onClick={()=>setMode(2)}>Hóa đơn</Dropdown.Item>
                        <Dropdown.Item onClick={()=>setMode(3)}>Ngày nhập</Dropdown.Item>
                    </DropdownButton>
