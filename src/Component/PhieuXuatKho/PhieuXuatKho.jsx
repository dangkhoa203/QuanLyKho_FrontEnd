...
export default function PhieuXuatKho(props){
    ...
    data={ nodes: data.nodes.filter((item) =>{
                switch (mode){
                    ...
                    case 3:
                        return  new Date(item.dateOfExport).toLocaleString('En-GB', {hour12: false}).toLowerCase().includes(search.toLowerCase())
                    default:
                        return item.id.toLowerCase().includes(search.toLowerCase())
    ...
    const COLUMNS = [
        ...
        {label: 'Ngày xuất kho', renderCell: (item) => new Date(item.dateOfExport).toLocaleString('En-GB', {
                year: "numeric",
                month: "2-digit",
                day: "2-digit",
                hour12: false
